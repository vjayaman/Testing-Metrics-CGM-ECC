...
checkColumns <- function(x, y, tol, it) {
  ...
  if (all(first_col, second_col, third_col)) {
    print(paste0("Test results and original results are same, within tolerance of ", tol))
  }else {
    print(paste0("Results do not match for iteration ", it))
    saveRDS(x, paste0("test_negs/iteration", it, "-actual.Rds"))
    saveRDS(x, paste0("test_negs/iteration", it, "-test.Rds"))
  }
}
```
...
for (it in 1:10) {
  ...
  incl_clusters <- datasetSizeBelow(td, 7000)
  ...
  strain_data <- read_tsv(params$strains) %>% 
    mutate(Date     = as.Date(paste(Year, Month, Day, sep = "-")), 
           Location = paste(Country, Province, City, sep = "_")) %>% 
    filter(Strain %in% rownames(td[[2]]))

  case1a <- actualResults(params$source, strain_data, c1$sigma, tau, gamma, typing_datum = td[[2]]) %>% 
  ...
  checkColumns(case1a, case1b, 1e-10, it)
}
